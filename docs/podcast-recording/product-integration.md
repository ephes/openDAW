...
| `cd packages/studio/core && npx vitest run` | 24 test files, 264 tests, all pass (includes `LongRecordingPeaksAdapter.test.ts`, `LongRecordingArtifact.probeAll` additions, `LongRecordingSampleLoader.test.ts`, `GlobalSampleLoaderManager.longRecordingFallback.test.ts` incl. the lazy-materialization case, `LongRecordingSession.test.ts` incl. the backpressure-cap and trip-then-stop race cases, `ProjectBundleLongRecording.test.ts` extension) |
